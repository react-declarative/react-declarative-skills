...
## Collection / MVVM Hooks

### useCollection
Observable reactive array of entities (each must have `id`). Only re-renders components that use changed data.

```ts
const collection = useCollection<T extends { id: string | number }>({
  initialValue: T[] | (() => T[]) | Collection<T>,
  onChange: (collection, target) => void,  // target = specific entity that changed, or null for bulk
  debounce: number,
});

// Methods (all async):
await collection.push(...items)
await collection.upsert(...items)        // insert or update by id
await collection.removeById(id)
await collection.removeAll()
await collection.setData(items)          // replace entire collection
await collection.clear()
collection.findById(id)                  // returns CollectionEntityAdapter
collection.find(fn), collection.filter(fn), collection.map(fn), collection.forEach(fn)
collection.toArray()                     // plain snapshot

// Entity-level update:
const entity = collection.findById('123');
await entity.setData({ value: entity.data.value + 1 });
await entity.setData(prev => ({ value: prev.value + 1 }));  // functional update
```

### useModel
Reactive single object. Re-renders on `setData`.
...
model.data                         // current snapshot
await model.setData({ key: val })  // merge partial
await model.setData(prev => ...)   // functional update
model.toObject()                   // plain snapshot
```

### useEntity
Like `useModel` but for records with an `id` field.

```ts
const entity = useEntity<T extends { id: string | number }>({
  initialValue: T | (() => T),
  onChange: (entity) => void,
});

entity.id                          // the entity's id
entity.data                        // current snapshot
await entity.setData({ ... })      // merge partial
```
